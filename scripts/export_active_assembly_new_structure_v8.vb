...
Dim fileName As String = System.IO.Path.GetFileNameWithoutExtension(sourcePath)
Dim numberMatch = System.Text.RegularExpressions.Regex.Match(fileName, "^(\d{3})_")
If Not numberMatch.Success Then
    Throw New Exception("Keine dreistellige Exponatnummer im Dateinamen gefunden: " & fileName)
End If

Dim outputPath As String = System.IO.Path.Combine(outputRoot, numberMatch.Groups(1).Value & ".stp")
...
MessageBox.Show("STP exportiert: " & outputPath, "You Decide")