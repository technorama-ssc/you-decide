...
foreach ($export in Get-ChildItem -LiteralPath $ExportRoot -File -Filter '*.stp') {
    if ($export.Name -notmatch '^(\d{3})\.stp$') {
        Write-Warning "Ignoring export with invalid name: $($export.Name)"
        continue
    }
