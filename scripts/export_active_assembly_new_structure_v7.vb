...
Dim finalPath As String = Nothing
...
If String.IsNullOrWhiteSpace(outputPath) Then
    ...
    finalPath = Global.System.IO.Path.Combine(hardwareDirectory, stepName)
    outputPath = Global.System.IO.Path.Combine(
        Global.System.IO.Path.GetTempPath(),
        "you-decide-" & Global.System.Guid.NewGuid().ToString("N") & ".stp")
End If
...
Dim hasSaveCopyOptions = translator.HasSaveCopyAsOptions(document, context, options)
translator.SaveCopyAs(document, context, options, data)

If Not String.IsNullOrWhiteSpace(finalPath) Then
    Global.System.IO.File.Copy(outputPath, finalPath, True)
    Global.System.IO.File.Delete(outputPath)
End If